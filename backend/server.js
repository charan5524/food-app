  ...
  for (let i = 0; i < retries; i++) {
    try {
      if (!process.env.MONGODB_URI) {
        ...
        console.error("MONGODB_URI=mongodb+srv://username:password@cluster.mongodb.net/food-app");
        return false;
      }
      ...
      console.error(`\n❌ MongoDB connection attempt ${i + 1} failed:`, err.message);
...
const startServer = async () => {
  ...
  if (connected) {
    ...
    console.warn("⚠️  MongoDB connection failed. Server starting without database connection.");
  }
...
if (process.env.NODE_ENV === "development") {
  ...
  app.post("/api/auth/reset-rate-limit", (req, res) => {
    try {
      ...
      res.json({ success: true, message: "Rate limit reset successfully. Please restart your server to fully clear the limit." });
    } catch (error) {
      console.error("Error resetting rate limit:", error);
      res.json({ success: true, message: "Please restart your server to clear the rate limit." });
    }
  });
  ...
  app.get("/api/auth/reset-rate-limit", (req, res) => {
    res.json({ 
      success: true, 
      message: "To reset rate limit, restart your backend server or wait 1 minute in development mode." 
    });
  });
}
